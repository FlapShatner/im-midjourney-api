import { MJConfig, WaitMjEvent, MJMessage, LoadingHandler, MJEmit, MJInfo, MJSettings, MJOptions, OnModal, MJShorten, MJDescribe } from './interfaces'
...
export class WsMessage {
 ...
 onSystem(event: 'ready' | 'messageCreate' | 'messageUpdate' | 'messageDelete' | 'interactionCreate' | 'interactionSuccess', callback: (message: any) => void) {
  this.on(event, callback)
 }
 private emitSystem(type: 'ready' | 'messageCreate' | 'messageUpdate' | 'messageDelete' | 'interactionSuccess' | 'interactionCreate', message: MJEmit) {
  this.emit(type, message)
 }
